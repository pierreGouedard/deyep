...
The table above show quality indicator of the estimator for different signal to noise ratio and $T=200$. Again, we see that the quality of the estimator drops as the theoretical precision between noisy bits and factor's bits are closed to each other. Finally we run the same experiment in the settings of joint sampled firing graph using again a measure grid of $n=1000$ bits from which we sampled randomly $\vert \mathcal{G}(f) \vert = 50$ bits linked to to target factor $f$ that activate with probability $p_f = 0.3$ and we set $p_N=0.6$. Finally we built the joint sampled firing graph by pre-selecting randomly 5 bits linked to the factor and running the sampling algorithm described previously using $p_{\mathcal{S}} = 1$.
