...
Throughout this section, we consider $G$ to be a firing graph with layer 0 composed by measure grid's bits $\mathcal{G}$ and $f \in \mathcal{F}$ denote some target factor that is linked to some bit of the measure grid. The distribution of activation of latent factors and measure grid's bits will be denoted  $d$ and $d'$ and the event "factor $f$ is active" will be denoted by $e$. \\
...
\Var \left[ s_{v,\rho, t, f}  \right] = (1 + \rho)^{2} \times \phi_{v, f} \times (1 - \phi_{v, f})
\end{equation}
...
Clearly, the complexity of the algorithm is dominated by the backward transmit and structure update operations. A standard worst case analysis of those operations gives $\mathcal{O}(n^{4} \times d_{max}^2)$. Yet this analysis relies on standard complexity time for dense matrix operations, and doesn't take into account sparsity of signals and direct link matrices or the randomness of inputs. In practice we have found that the forward and backward propagation of the bits and factors activation is time consuming, especially when both $N$ and $T$ are large numbers. As a consequence, batch\_size successive bits and factor's state are forward and backward propagated through the graph at each iteration. The decrease in time complexity of this practical trick is impressive and worth the slight gain in space comlexity of the algorithm even though it does not stick to the expectation of theoretical worst case analysis. Finally this trick requires to dynamically change the batch\_size so that treshold for the number of updates at each edges is respected.
...
\begin{align*}
\mathbb{P} \left( E \right)&= \sum_{j=0}^{\vert S \vert} p^j \times q^{\vert S \vert -j} \times\mathbb{P}_{\mathcal{S}_i} \left( \vert \lbrace s \in S \setminus \omega_{I \cup \{s\}, i + 1, f } < \omega \rbrace \vert = j \right) \\
...
\begin{equation}
\mathbb{P} \left( \omega_{v, f} > \omega \right) \leq C \times \max \left( \exp\left(- t \times \left(\frac{\delta^{-}\delta_{i,f} c}{\sigma}\right)^2 \right), \exp\left(- t \times \delta^{-}\delta_{i,f}c \right) \right)
\end{equation}
...
\begin{leftbar}
\textbf{Proof.} First recalling the topology of the firing graph $G$ built at sampling step, again, using a similar argument (\ref{prop:2layer-2}) than in the previous proof, for each sampled bits $s\in S$ and each input vertex $v(\{ s\}, 1)$ of $G$ $\omega_{v, f} = \omega_{I \cup  \{s\}, i+1, f}$. Furthermore 

\begin{equation*}
\mathbb{P} \left( \omega_{v, f} >  \omega \right) = \mathbb{P}_{\mathcal{S}} \left( \omega_{I \cup \{s\}, i + 1, f } < \omega \right) \times \mathbb{P} \left( s_{v, f}(N, T, \rho) > 0 \vert \omega_{I \cup \{ s \}, i + 1, f} > \omega \right)
\end{equation*}

Where $s \in S$ and $v(I \cup \{s\}, i+1)$ a vertex of layer 1 of a firing graph with layer 0 composed of $I \cup \{s\}$. Then, we choose the value of the postive real $\omega$ such that there exists a bit $b^{-}$ that verifies
...
\begin{equation*}
\mathbb{P} \left( s_{v, f}(N, T, \rho_i) > 0 \vert \omega_{v, f} > \omega \right) \leq \mathbb{P}\left(\vert \sum_{t=0}^{T_i - 1} s_{v, \rho, t, f} - \mathbb{E}\left[s_{v, \rho, t, f}\right] \vert > t \times \delta^{-} \delta_{i,f} \right)
\end{equation*}

With $\delta_{i, f} = (\rho +1) \times \phi^{2} \times \frac{1 - \Vert \mathcal{P}_{f} \Vert_{d'}}{\Vert \mathcal{P}_{f} \Vert_{d'}}$. Then using the Chernoff inequality as written in \cite{TAO-1} using $\lambda = \sigma^{-1} \delta^{+} \delta_{i,f}$ we obtain


\begin{align*}
\mathbb{P}\left(\vert \sum_{t=0}^{T - 1} s_{v, \rho, t, f} - \mathbb{E}\left[s_{v, \rho, t, f}\right] \vert > T \times \mu_{v, f}\delta^{-} \rho \right) \leq C \times \max \left( \exp\left(- t \times \left(\frac{\delta^{-}\delta_{i,f} c}{\sigma}\right)^2 \right), \exp\left(- t \times \delta^{-}\delta_{i,f}c \right) \right)
\end{align*}
...
Thus its expected size is $\mathbb{E}\left[ \vert S \vert \right] = k + (n - k) \times p_N \times p_{\mathcal{S}} $. Furthermore if $I= \{b_1^{*}, \ldots, b_i^{*} \}\in S(\mathcal{f})$ a set of pre-selected bits and $S$ is a set of bits sampled using $\mathcal{S}$, then, there exists a positive real $\omega_i$ such that
...
The signal plus noise model is implemented in python and mainly use standard numpy module to generate random signal that fit its probabilistic framework, more details about the implementation can be found in appendix B. The simulation of this signal plus noise requires the implementation of random signals that respect probabilistic properties of the model. In our case we generate $n=1000$ bits that randomly activate with probbility $p_N$ and we choose randomly $\vert  \mathcal{G}(f)\vert = 50$ bits that are linked to a latent factor that activate with probability $p_f=0.3$. Finally we built the single sampled firing graph running the sampling algorithm described previously using $p_{\mathcal{S}} = 1$.
...
In every experiement, following the procedure described in the previous section we end with $\rho=1$. Furthermore, each of the figure above shows the values of the score process through the tracking of the weight of edges of a single sampled firing graph. Blue lines show the weight of edges outcoming from input vertices whose activation corresponds to activation of some bits $b\in \mathcal{G}(f)$. Red lines correspond to the weight of edges outcoming from input vertices whose activation corresponds to noisy activation. Finally the black horizontal line represents the theoretical mean value of of the score process, with respect to $f$, with parameter $N, T, \rho$ chosen as described previously, of an edge outcoming from a input vertex that activates as a bits of the grids linked to $f$. As theoretical modelisation suggests we have a clear separation between the two kind of bits and a convergence toward theoretical mean for process corresponding to factors activation. Yet the higher is the noisy activation probability the more difficult it is to separate the tow category of bits sampled. In order to highlight it we run a new simulation with $n=1000$, $\vert \mathcal{G}(f)\vert = 50$, $p_f=0.3$, $T=200$ and $p_{\mathcal{S}}$. Yet at the end of the draining we choose all the input vertices of the firing graph that stil have outcoming edges, and use their combined activation as estimator of the factor's activation measuring their precision and recall over $100$ repetion for each SNR ratio
...
The table above show quality indicator of the estimator for different SNR ratio. the two first columns gives respectively the mean and standard deviation of the precision of the estimator. The two following columns are respectively the mean and the standard deviation of the recall of the estimator. Finally the last column is the number of experiments that ended without any remaining input vertex, so that the construction of an estimator is not possible. Again, we see that the quality of the estimator drops as the theoretical precision between noisy bits and factor's bits are close to each other. Yet it reveal that this naive estimator, for a reasonable SNR ratio, is still efficient to predict the activation of target latent factor. Finally we run the same experiment in the settings of joint sampled firing graph using again a measure grid of $n=1000$ bits from which we sampled randomly $\vert \mathcal{G}(f) \vert = 50$ bits linked to to target factor $f$ that activate with probability $p_f = 0.3$ and we set $p_N=0.6$. Finally we built the joint sampled firing graph by pre-selecting randomly 5 bits linked to the factor and running the sampling algorithm described previously using $p_{\mathcal{S}} = 1$.
...
In this case, following the procedure described previously we end with $N=7$, $\rho=7$ and $\omega_5 \simeq 0.062$, those value of $\rho$ the gain in variance of score process that we observe. In this case we are more likely to drain vertex that activate corrrectly with factors and we are more likely to have noisy vertex remaining after the draining.
...
Here again, the sparse measure grid model is implemented using python and the standard numpy.random module to generate random signal that fit its probabilistic framework. In our case we generate $n=1000$ bits. Then for each of the $K=10$ latent factor we link measure grid's bits independently with probabiity $p_g=0.3$. Furthermore each latent factor has the same probability $p_f=0.3$ to activate. Finally we built the single sampled firing graph running the sampling algorithm described previously using $p_{\mathcal{S}} = 1$ and we set $\omega= \omega_{10}$ the lowest purity coefficient for a bits that is linked to the target factor.
...
We clearly see a rapid differentiation of score process according to their purity rank. We can also observe that the lower the coefficient the closer are the score process of corresponding edges. Finally the behaviour of score processus validate the efficiency of the draining algorithm to rank bits of the measure grids blindly in a attempt to identify latent factor.\\
...
\begin{align*}
\psi_{v f} = \frac{\langle \mathcal{P}^{l}_I, \mathcal{P}_{*} \rangle_{d, e}}{\Vert \mathcal{P}_{f} \Vert_{d'}} = \frac{\langle \mathcal{P}^{l}_I, \mathcal{P}_{*} \rangle_{d \vert e} \times \Vert \mathcal{P}_f \Vert_{d'}}{\Vert \mathcal{P}_{f} \Vert_{d'}} = \mu_{v, f} 
\end{align*}
...
the proof come with identification of terms.
...
&= t \times \Vert \mathcal{P}_v \Vert_{d \vert \bar{e}} \times \Vert \vert_{d'}
\end{align*}
...
Which is the definition of $\phi_{v, f}$.
