...
Where $\circ$ denotes the usual Hadamard product, $x_f^{(t-d)}$ is the vector of states of factors at instant $t-d$ and $p, q$ are predifined positive integer. A correct backpropagation of $x_{b,o}^{(t)}$ up to the input vertices is made possible by using time synchronisation and topology of the firing graph. We denote by $V_i$, $i\in \mathbb{N}$ the set of vertices that has a path composed of $i$ vertex toward an output vertex. Let $G$ be a firing graph with $k \in \mathbb{N}^{*}$ layers augmented with a layer of ouptut vertices. Let $V_o$ the set of output vertices, $\forall v, o \in V_0 \times V_o$,  $v$ is elligible to $o$'s feedback at instant $t$ if and only if 
