...
Given a firing graph with layer 0 composed of measure grid's bits, we have seen that the propagation of activations induces a stochastic process at each vertex that described its state. Here we introduce some more complex stochastic processus at each vertex of $G$. Given a vertex $v$ at layer $k \geq 0$, its  characteristic polynome $\mathcal{P}_{v}$, a factor $f \in \mathcal{F}$ and e the event "factor $f$ is active" we define the score process with respect to factor $f$ as

\begin{equation*}
s_{v,f}\left[N, T, p, q \right] = N + \sum_{t=1}^{T} s_{v, p, q, t, f}
\end{equation*}

Where $(N, T, p, q) \in \mathbb{N}^4$ and $\lbrace s_{v, p, q, t, f} \rbrace_{t \in \mathbb{N}}$ a set of i.i.d random variable. $s_{v, p, q, t, f}$ takes value $q$ if the event e was true at time $t - k $ and value $-p$ if it was false, given that $v$ activate at $t$. That is, $\forall$ $t  < k $, $s_{v, p, q,t, f} = 0$ and $\forall$ $t  \geq k$

\begin{equation*}
s_{v, p,q, t, f} = \begin{cases} q, & \text{with probability } q_s  \\ -p, & \text{with probability } 1 - q_s \end{cases}
\end{equation*}

Where $q_r = \Vert \mathcal{P}_v \Vert_{d,e} $, $q_p = \Vert \mathcal{P}_v \Vert_{d} - q_r$ and $q_s = \frac{q_r}{q_r + q_p}$.  $d, e$ is the distribution over measure grid's activation that intersect with the event e
...
If $s_{v, f}[N, T, p, q]$ denotes the score process of $v(I, l)$ with respect to $f$ with $N, T, p, q \in \mathbb{N}^4$, then
...
Where $\circ$ denotes the usual Hadamard product, $x_f^{(t-d)}$ is the vector of states of factors at instant $t-d$ and $p, q$ are predifined positive integers. A correct backpropagation of $x_{b,o}^{(t)}$ up to the input vertices is made possible by using time synchronisation and topology of the firing graph. We denote by $V_i$, $i\in \mathbb{N}$ the set of vertices that has a path composed of $i$ vertex toward an output vertex. Let $G$ be a firing graph with $k \in \mathbb{N}^{*}$ layers augmented with a layer of ouptut vertices. Let $V_o$ the set of output vertices, $\forall v, o \in V_0 \times V_o$,  $v$ is elligible to $o$'s feedback at instant $t$ if and only if 
...
For some real $\delta > \delta^{+} > 0$. Then, We choose the 4-tuple $(N, T, p, q)$ as follow:
...
Now, let $v$ such that $\omega_{v,f}  < \omega$ one can write

\begin{align*}
\mathbb{P} \left( s_{v,f}[N, T, p, q] < 0 \right) &= \mathbb{P} \left( N_i + \sum_{t=1}^{T} s_{v, p, q, T, f} < 0 \right) \\
...
\begin{align*}
\mathbb{P} \left( s_{v,f}[N, T, p, q] < 0 \right) &= \mathbb{P}\left( \sum_{t=0}^{T - 1} s_{v, p, q, t, f} - \mathbb{E}\left[s_{v, p, q, t, f}\right] < -T \times (p + q) \times  \delta_{v, f} \right)\\
&\leq \mathbb{P}\left(\vert \sum_{t=0}^{T - 1} s_{v, p, q,t, f} - \mathbb{E}\left[s_{v, p, q, t, f}\right] \vert > T \times (p + q) \times  \delta_{v, f} \right)\\
&\leq \mathbb{P}\left(\vert \sum_{t=0}^{T - 1} s_{v, p, q, t, f} - \mathbb{E}\left[s_{v, p, q, t, f}\right] \vert > T \times \delta_{f} \right)
\end{align*}

With $\delta_{f} = (p + q) \times \delta^{+} \times \phi^{2} \times \frac{1 - \Vert \mathcal{P}_{f} \Vert_{d'}}{\Vert \mathcal{P}_{f} \Vert_{d'}}$. \\

At this point we have to notice that $\lbrace s_{v, p, q, t, f} \rbrace_{t=1, \ldots, T - 1}$ is a sequence of i.i.d random variables with mean $\mu$ and variance $\sigma^2$ that verifies $\vert s_{v, p, q, t, f} \vert \leq max(p, q)$. Thus one can apply the Chernoff inequality as formulated in \cite{TAO-1}. In particular, taking $\lambda = \sigma^{-1}\delta_{f}$ we obtain

\begin{align*}
\mathbb{P}\left(\vert \sum_{t=0}^{T - 1} s_{v, p, q, tn f} - \mathbb{E}\left[s_{v, p, q, t, f}\right] \vert > T \delta_{f} \right) &= \mathbb{P}\left(\vert \sum_{t=0}^{T - 1} s_{v, p, q, t} - \mathbb{E}\left[s_{v, p, q, t}\right] \vert > \lambda\sigma \sqrt{T} \right)\\
&\leq C \times \max \left( \exp\left(- T \times \left(\frac{\delta_{f} c}{\sigma}\right)^2 \right), \exp\left(- T \times \delta_{f}c \right) \right)
\end{align*}


With $C, c$ some positive constant and $\Var[s_{v, p, q, t, f}] = \sigma^2$ for $t \in \{0, \ldots, T-1 \}$. Q.E.D.
...
\begin{theorem}
\label{th_precision}
Given a set of sampled bits $S$, a set of pre-selected bits $I =\{b_{1}^{*}, \ldots, b_{i}^{*}\}$ a target factor $f$ and $G$ the firing graph built after sampling algorithm. A sequence of 5-tuple $(\omega, N, T, p, q)$ exists such that for each remaining input vertex $v$ of the drained firing graph verifies 
...
Now, let $v$ such that $\omega_{v,f}  > \omega$, reproducing the same development as it was done in the proof of previous theorem, one can derive a convenient form to easily apply the Chernoff inequality.
...
The combination of theorems shows that the association of sampling and draining with the right choice of 5-tuple $(\omega, N, T, p, q)$ gives a convenient tool to select measure grid's bits with purity coefficient lower or equal to $\omega$. Furthermore when $t \rightarrow +\infty$ the identification is almost certain, which highlights the trade-off between efficiency and complexity of the algorithm embedded in the choice of $\omega$ and $T$, on which depends $N, p$ and $q$.
...
\begin{align*}
(p, q) &\in \mathbb{N}^{2} \textit{ such that } \phi_i \times (p + q) - p < 0 \textit{ and } \phi_i' \times (p + q) - p < 0 \\
N &= -T \times (\phi \times (p+q) - p)\\
T &\in \mathbb{N} \textit{ such that } N \textit{ large enough}
\end{align*}
...
In this case, we obtain $N=7$ and $\omega_5 \simeq 0.062$ when following the procedure described in previous section. As for the first experiment, blue lines show the weight of edges outcoming from sampled bit $b \in \mathcal{G}$ and red lines correspond to the weight of edges outcoming from sampled bit $b \notin \mathcal{G}$. The black horizontal line represents the theoretical mean value of $s_{v, f}[N, T, p, q]$ of a vertex with characteristic polynome $\mathcal{P}_v = \mathcal{P}_{\{b_1, \ldots, b_5, b\}, 5}$, with $\{b_1, \ldots, b_5\}$ the set of pre selected bits and $b \in \mathcal{G}(f)$. The simulation validate the expectation from theory and the high value of $p+q$ explains the larger variance of score process.

\section{Case of sparse measure grid}

This particular case is a more complex than the previous one. We first define the statistical signature of factor and bit's activation. Then we quantify the sampling strategy and arg a choice for the 4-tuple $(\omega, T, N, p, q)$. Finally we present simulations and provide discussion of the results obtained with this sparse case.
...
\caption{Observation of sampled bit's score processus in a joint sample firing graph $i=5$, $T=500$ and $(p, q) = (1, 1)$}
\label{fig:sim_si_2}
\end{figure}
...
Finally, in the same context, if $s_{v, f}[N, T, p, q]$ denotes the score process of $v$ with respect to $f$ as defined in section 1.2.2 with some $N, T, p, q \in \mathbb{N}^4$, the result (\ref{prop:score_mean}) stands that
