...
class TestSolver(unittest.TestCase):
    def setUp(self):
        ...
        self.and_dn = DeepNetwork.from_matrices(mat_net, mat_in, mat_out, 100, w0=self.wo, l0=self.l0, tau=self.tau)
        ...
        self.xor_dn = DeepNetwork.from_matrices(mat_net, mat_in, mat_out, 100, w0=self.wo, l0=self.l0, tau=self.tau)
        ...
        self.tree_dn = DeepNetwork.from_matrices(mat_net, mat_in, mat_out, 100, w0=self.wo, l0=self.l0, tau=self.tau)
    ...
    def test_xor_pattern(self):
        ...
        # Create solver
        self.xor_dn.graph['Ow'] *= 3
        solver = DeepNetSolver(self.xor_dn, self.xor_pat.delay, imputer, 1)
        solver.run_epoch(n=500)
        ...
        #                                            len(self.xor_pat.network_nodes), len(self.xor_pat.output_nodes),
        #                                            weights=[10] * len(self.xor_pat.build_graph_pattern_init()))
        # xor_dn = DeepNetwork.from_matrices(mat_net, mat_in, mat_out, 100, w0=self.wo, l0=self.l0, tau=self.tau)
    ...
    def test_tree_pattern(self):
        ...
        # Create solver
        solver = DeepNetSolver(self.tree_dn, self.tree_pat.delay, imputer, 1)
        l_t_epoch, l_t_forwardp, l_t_forwardt, l_t_backwardp, l_t_backwardt = solver.run_epoch(50, evaluate_running_time=True)
        ...
        # EXPECTED FINAL GRAPH
        # tree_dn = DeepNetwork.from_matrices(mat_net, mat_in, mat_out, 100, w0=self.wo, l0=self.l0, tau=self.tau)
