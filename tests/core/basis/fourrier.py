...
class TestFourrierBasis(unittest.TestCase):
    def setUp(self):
        ...
        self.deep_network_1 = DeepNetwork.from_matrices('test', mat_net, mat_in, mat_out, self.capacity, 'fourrier')
    ...
    def test_fourrier_basics(self):
        ...
        self.assertEqual(np.round(np.real(s_net1.dot(s_net2.conjugate()))), 0.)
        ...
        self.assertEqual(np.round(np.real(s_out.dot(Basis.base.conjugate()))), 1.)
        ...
        self.assertEqual(np.round(np.real(s_out.dot(Basis.base.cojugate()))), 1.)
        ...
        self.assertEqual(np.round(np.real(s_in1_.dot(s_in1.conjugate()))), 1.)

        # Test decoding double encoding
        s_in2_ = Basis.decode(s_out_, t=3)
        self.assertEqual(np.round(np.real(s_in2_.dot(s_in2.conjugate()))), 2.)

    def test_fourrier_limits(self):
        """
